    ...
    init:function() {
        ...
        pr_summary.reseted = false;
        pr_summary.ui_history = [];
        pr_summary.reset();
        ...
        $('.prompt_button').on("click", function() {
            ...
            pr_summary.ui_history.push($(this).attr("id"));
            $("#prompt").val(prompt);
    ...
    checkSession:function() {
        ...
        $.each(r, function(index, val) {
            let content = val.split('=');//split('=')を使用しcookie名と値に分ける
            if (content[0].trim() == 'sessionid') {
                found = true;
                return false ;
            }
        })
    ...
    getSummary:function() {
        if (!pr_summary.checkSession()) {
            // セッション切れ
            window.location.href = '/login/?redirect=general_voxt';
        }
        ...
        let param = {'prompt':prompt, 'text':text, 'mode':mode, 'history':JSON.stringify(pr_summary.ui_history)};
        Object.assign(param, pr_summary.testmode);
        $.post(params.path + "/summary", param)
            .done(function(data) {
                $('.summarize_button').prop("disabled", false);
                pr_summary.ui_history = [];
    ...
    getDataProgressive:function(requestId) {
        $.post(params.path + "/getprogress", {'requestId':requestId, 'canceled':pr_summary.canceled ? 1 : 0})
            ...
            if (!data.result || pr_summary.requestId != requestId) {
                ...
                toastr.error("ご迷惑をおかけしております。通信エラーが発生しました。", '', {timeOut: 0, extendedTimeOut:0, closeButton: true})
                pr_summary.requestId = "";
                pr_summary.showLoader(false);
            }
            console.log(data);
