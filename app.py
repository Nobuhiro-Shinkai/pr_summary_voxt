...
def getExceptionMsg(exception):
	if isinstance(exception, openai.error.Timeout):
		...
		return "ご迷惑をおかけしております。現在OpenAIのサービスをご利用いただくことができません。1、2分ほどお待ちいただいた後、再度お試しください。"
	elif isinstance(exception, openai.error.ServiceUnavailableError) or isinstance(exception, openai.error.APIConnectionError):
		return "ご迷惑をおかけしております。現在OpenAIのサービスをご利用いただくことができません。復旧するまでお待ちいただくようお願い申し上げます。"
	else:
		return "ご迷惑をおかけしております。原因不明のエラーが発生しました。"


def dosummary(requestId, text, prompt, mode, ip, config, city, user):
	...
	try:
		...
		try:
			...
			promptToken = handler.encoding.encode(p + text)
			...
			handler.count += len(handler.encoding.encode(res))
			...
			logger_.debug(f"summary IP:{ip}, requestId:{requestId}, input_tokens:{len(promptToken)}, output_token:{handler.count}, execute_time:{span.total_seconds()}")
...
def summary():
	...
	history = request.values['history']
	...
	logger_.debug(f"summary IP:{ip}, requestId:{requestId}, UI history:{history}")
	logger_.debug("summary - progressData_ is created:{}".format(requestId))

	# start 1 worker processes
	thread = threading.Thread(target=dosummary, args=(requestId, text, prompt, mode, ip, application.config, session['city'], session['user']))
	thread.start()
	return jsonify({'result':True, 'requestId':requestId})

def updateCookie(res):
	max_age = 60 * 30 # 30min.
	expires = int(datetime.datetime.now().timestamp()) + max_age
	#logger_.debug(f"redirect_url : {redirect_url}")
	sessionId = request.cookies.get('sessionid')
	logger_.debug(f"sessionid:{sessionId}")
	res.set_cookie("sessionid", value=sessionId, max_age=max_age, expires=expires, domain=request.host, secure=True)
	return res
	...
	try:
		...
		if requestId in progressData_ and progressData_[requestId].qsize() > 0:
			...
			if 'error' in data and data['error']:
				return updateCookie(jsonify({'result':True, 'error':data['error'], 'errormsg':data['errormsg']}))
			else:
				if 'reset' in data:
					return updateCookie(jsonify({'result':True, 'reset':data['reset'], 'progress':data['progress'], 'total':data['total'], 'is_final':data['is_final']}))
				else:
					return updateCookie(jsonify({'result':True, 'summary':data['summary'], 'end':data['end']}))
		else:
			return updateCookie(jsonify({'result':True}))
	except:
		logger_.error('getProgress', exc_info=True)
		return updateCookie(jsonify({'result':False}))	
